...
from cx_connectors.sources.alphavantage import AlphaVantageOptionsSource, AlphaVantageSource
...
def test_stooq_raises_on_antibot_html():
    ...
    s = StooqSource(
        "IBM", session=_FakeSession([_CsvResponse("<!DOCTYPE html><html>challenge</html>")])
    )
...
def test_alphavantage_prices_sorted_ascending():
    ...
    header, rows = AlphaVantageSource(
        "IBM", "KEY", session=_FakeSession([_JsonResponse(payload)])
    ).read()
...
def test_alphavantage_rate_limit_raises():
    ...
    s = AlphaVantageSource(
        "IBM", "KEY", session=_FakeSession([_JsonResponse({"Note": "rate limit"})])
    )
    with pytest.raises(ValueError):
        s.read()
...
def test_alphavantage_options_contracts():
    ...
    contracts = AlphaVantageOptionsSource(
        "IBM", "KEY", session=_FakeSession([_JsonResponse(payload)])
    ).read_contracts()
...
def test_build_options_wall_shape_and_filtering():
    obj = build_options_wall(
        _PRICE_HEADER, _PRICE_ROWS, _CONTRACTS, symbol="IBM", expiry="2026-10-17"
    )
    cfg = obj["config"]
...
def test_fetch_options_wall_alphavantage_wires_both_sources():
    ...
    obj = fetch_options_wall(
        "IBM", api_key="KEY", provider="alphavantage", session=session, expiry="2026-10-17"
    )
...
def test_fetch_stooq_requires_option_contracts():
    ...
    with pytest.raises(ValueError):
        fetch_options_wall(
            "IBM", provider="stooq", session=_FakeSession([_CsvResponse(_STOOQ_CSV)])
        )
...
def test_build_options_wall_multi_groups_by_expiry():
    ...
    for exp in ("2026-07-17", "2026-08-21"):
        for k in (95, 100, 105):
            contracts.append(
                {"type": "call", "strike": k, "expiration": exp, "mark": max(0.5, 100 - k + 3)}
            )
            contracts.append(
                {"type": "put", "strike": k, "expiration": exp, "mark": max(0.5, k - 100 + 3)}
            )
    multi = build_options_wall_multi(
        ["Date", "Open", "High", "Low", "Close"],
        rows,
        contracts,
        symbol="X",
...
def test_build_options_wall_multi_calendar_keeps_weeklies():
    ...
    build_options_wall_multi(
        hdr_ohlc(), rows, contracts, symbol="X", select="liquid", min_contracts=2, max_expiries=6
    )
    cal = build_options_wall_multi(
        hdr_ohlc(),
        rows,
        contracts,
        symbol="X",
